...
export function clock() {
    ...
    function createTimezoneListItems(list) {
        for (let index = 0; index < jsonData.length; index++) {
            ...
            item.innerHTML = `<span>${jsonData[index]?.city}</span>, ${jsonData[index]?.country}`
        }
    ...
    function sortItemsAlphabetically(list) {
        Array.from(list.children)
            ...
            .forEach(li => {
                li.setAttribute("group-by-letter", `${li.textContent.charAt(0).toUpperCase()}`)
                list.appendChild(li)
            })
    }
    ...
    function wrapElements() {
        const timezoneItems = document.querySelectorAll("[group-by-letter]")
        ...
        timezoneWrappers.forEach(wrapper => {
            timezoneItems.forEach(item => {
                if (item.getAttribute("group-by-letter") === wrapper.getAttribute("id")) {
